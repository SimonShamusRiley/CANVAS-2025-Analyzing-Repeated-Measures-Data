...
Proc HPMIXED data=Selected lognote;
	...
	Random  Block  Trt*Block  Trt*Year*Block Trt*Year*DAP*Block;
...
libname xl xlsx "Model_complete_repeated_Depth.xlsx";
	Data xl.Cout; set Cout;run;
	Data xl.AOV;set AOV;run;
libname xl clear;

Data AGR.Mcomplete_rep_Depth_cov; set cout;run;
...
proc tabulate data=cout1;
	...
	class system row col; 
	var estimate;
	Table row, System*col*estimate*sum/nocellmerge;
run;
ods excel close;
